...
// AdminProjectConfig mirrors the payload returned by admin /api/configs endpoints.
...
// NotificationsRuntimeConfig carries notification runtime overrides for Lark.
type NotificationsRuntimeConfig struct {
	Region  string `json:"region"`
	Webhook string `json:"webhook"`
}

// DecodeValue decodes config_value into a typed structure.
...
// ListAdminProjectConfigs fetches all configs and lets callers filter by config_type.
...
// ListAdminProjectConfigsByType fetches configs filtered by config_type.
// It first tries the query API and falls back to local filtering when needed.
func ListAdminProjectConfigsByType(
	...
	endpoint := fmt.Sprintf(
		"%s/api/configs/type/%s",
		NormalizeAdminBaseURL(adminBaseURL),
		url.PathEscape(trimmedType),
	)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, endpoint, nil)
	if err != nil {
	...
	// Backward compatibility: old admin may not support query filtering.
...
// LoadSingleAdminProjectConfigByType loads exactly one config by type.
...
// LoadModelRuntimeConfig loads and validates a model runtime config by type.
...
// LoadNotificationsRuntimeConfig loads and validates a notification runtime config by type.
func LoadNotificationsRuntimeConfig(
	ctx context.Context,
	adminBaseURL string,
	timeoutSecond int,
) (*NotificationsRuntimeConfig, error) {
	cfg, err := LoadSingleAdminProjectConfigByType(
		ctx,
		adminBaseURL,
		timeoutSecond,
...
// adminHTTPClient creates a short-lived HTTP client with caller-defined timeout.
