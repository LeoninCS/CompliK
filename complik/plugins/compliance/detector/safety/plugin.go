...
func (p *SafetyPlugin) loadConfig(setting string) error {
	...
	if strings.TrimSpace(p.safetyConfig.APIKey) == "" ||
		...
		return errors.New("model runtime config from admin is incomplete")
	}
...
func (p *SafetyPlugin) applySafetyPromptRules(ctx context.Context) error {
	...
	p.log.Info("Applied safety rules from admin", logger.Fields{
...
func (p *SafetyPlugin) applyModelRuntimeConfig(ctx context.Context) error {
	// Pull shared model runtime settings from admin (required in pure-admin mode).
