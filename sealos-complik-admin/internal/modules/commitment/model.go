...
func AutoMigrate(db *gorm.DB) error {
	if db == nil {
		return errors.New("commitment automigrate: database is nil")
	}

	if db.Migrator().HasColumn(&Commitment{}, legacyNamespaceColumn) &&
		!db.Migrator().HasColumn(&Commitment{}, "namespace") {
		if err := db.Migrator().
			RenameColumn(&Commitment{}, legacyNamespaceColumn, "namespace"); err != nil {
