...
func AutoMigrate(db *gorm.DB) error {
	if db == nil {
		return errors.New("unban automigrate: database is nil")
	}

	if db.Migrator().HasColumn(&Unban{}, legacyNamespaceColumn) &&
		!db.Migrator().HasColumn(&Unban{}, "namespace") {
		if err := db.Migrator().
			RenameColumn(&Unban{}, legacyNamespaceColumn, "namespace"); err != nil {
