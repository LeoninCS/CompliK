...
func SendGlobalBatchAlert(results []*NamespaceScanResult, webhookURL, region string) error {
	if webhookURL == "" {
	...
	for idx, r := range results {
		...
		for processIdx, p := range r.ProcessInfos {
			allElements = append(
				allElements,
				newMarkdownElement(formatProcessSection(processIdx+1, p)),
			)
		}
	}
...
func formatSummarySection(region, nodeName string, totalProcesses, namespaceCount int) string {
	lines := []string{
		"**告警总览**",
		"可用区：" + quoteValue(displayValue(region, "未配置")),
		"节点名称：" + quoteValue(displayValue(nodeName, "未知")),
		"告警时间：" + quoteValue(time.Now().Format(time.RFC3339)),
...
func buildProcessAnalysis(info *models.ProcessInfo) []string {
	...
	if strings.Contains(message, "process name") || strings.Contains(message, "进程名") {
		analyses = append(
			analyses,
			"当前通过进程名命中黑名单规则，可能是已知恶意程序，也可能是调试、巡检或运维命令导致的误报，建议结合镜像来源和启动者继续核查。",
		)
	}

	if strings.Contains(message, "command line") || strings.Contains(message, "keyword") ||
		strings.Contains(message, "命令行") ||
		strings.Contains(message, "关键词") {
		analyses = append(analyses, "当前通过命令行关键字命中规则，可能涉及挖矿、反弹 Shell、扫描探测等行为，也可能是测试命令或安全演练触发。")
	}

	if isUnknownValue(info.Namespace) || isUnknownValue(info.PodName) ||
		isUnknownValue(info.ContainerID) {
		analyses = append(
			analyses,
			"容器元数据未完整获取，可能是容器已退出、容器运行时接口暂时不可用、当前进程属于宿主机，或该进程并非 Kubernetes 托管工作负载。",
		)
	}
...
func translateReason(message string) string {
	...
	if after, ok := strings.CutPrefix(message, "命令行命中关键词黑名单规则 '"); ok {
		rule := strings.TrimSuffix(after, "'")
		return "进程命令行命中关键字黑名单规则 " + quoteValue(rule)
	}
